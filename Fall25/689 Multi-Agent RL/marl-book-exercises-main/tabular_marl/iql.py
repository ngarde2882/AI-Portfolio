    ...
    def act(self, obss) -> List[int]:
        ...
        for i, obs in enumerate(obss):
            if random.random() < self.epsilon:
                ...
                # q_values = [self.q_tables[i][str((obs, a))] for a in range(self.n_acts[i])]
                # max_q = max(q_values)
                # max_actions = [a for a, q in enumerate(q_values) if q == max_q]
                # actions.append(random.choice(max_actions))
                actions.append(np.argmax([self.q_tables[i][str((obs, a))] for a in range(self.n_acts[i])]))
        return actions
        ...
        if done:
            for i in range(self.num_agents):
                for j in range(len(obss)):
                    Q[i][str((obss[j], actions[j]))] = Q[j][str((obss[j], actions[j]))] * (1-self.learning_rate) + self.learning_rate * rewards[j]
        else:
            for i in range(self.num_agents):
                for j in range(len(obss)):
                    Q[i][str((obss[j], actions[j]))] = Q[j][str((obss[j], actions[j]))] * (1-self.learning_rate) + self.learning_rate * (rewards[j] + self.gamma * max([Q[j][str((n_obss[j], a))] for a in range(self.n_acts[j])]))
        return Q[i][str((obss[i], actions[i]))]

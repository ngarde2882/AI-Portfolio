...
class ACConfig:
    ...
    switch_decay_seconds: float = 8.0    # linearly decay to 0 over this many seconds (reduced from 25s)
    lr: float = 3e-4
...
class ACProfilePolicy:
    ...
    def __init__(self, profile_names: List[str],
        ...
        # --- training buffer (A2C with per-goal segments + per-tick rewards) ---
        self.training = True
        self._train_obs: List = []
        self._train_acts: List = []
        self._train_vals: List = []
        # Index into _train_obs where the current goal-segment begins
        self._segment_start: int = 0
        # Accumulated per-tick rewards since the last goal (or match start)
        self._tick_reward_accum: float = 0.0
    ...
    def add_tick_reward(self, r: float) -> None:
        """Accumulate a per-tick reward into the current goal segment.

        Called once per game tick by the MatchRunner with a small ball-half signal.
        The total accumulates until finalize_goal() or finalize_match() is called.
        """
        if self.training:
            self._tick_reward_accum += r

    def finalize_goal(self, goal_reward: float) -> None:
        """Train on all decisions since the last goal, then start a fresh segment.

        Called by MatchRunner each time a goal is scored.  The segment return is
        accumulated_tick_rewards + goal_reward (±1).  After training, _segment_start
        advances to the current buffer length so the next kickoff is independent.
        """
        if (not self.training) or (self.net is None) or (self.opt is None):
            self._segment_start = len(self._train_obs)
            self._tick_reward_accum = 0.0
            return
        seg_end = len(self._train_obs)
        if seg_end > self._segment_start:
            batch_obs = np.asarray(self._train_obs[self._segment_start:seg_end], dtype=np.float32)
            acts_idx  = np.asarray(self._train_acts[self._segment_start:seg_end], dtype=np.int64)
            vals      = np.asarray(self._train_vals[self._segment_start:seg_end], dtype=np.float32)
            total_return = float(self._tick_reward_accum) + float(goal_reward)
            returns = np.full_like(vals, total_return, dtype=np.float32)
            adv     = returns - vals
            self.step_update(batch_obs=batch_obs, acts_idx=acts_idx, returns=returns, adv=adv)
        self._segment_start = seg_end
        self._tick_reward_accum = 0.0

    def finalize_match(self, outcome_reward: float = 0.0) -> None:
        """Process the remaining segment after the last goal (or whole match if no goals).

        outcome_reward is no longer the primary learning signal — per-goal ±1 via
        finalize_goal() handles that.  Pass 0.0 at match end so the final segment
        trains only on accumulated tick rewards.  The buffer is cleared afterwards.
        """
        if (not self.training) or (self.net is None) or (self.opt is None):
            self._train_obs.clear(); self._train_acts.clear(); self._train_vals.clear()
            self._segment_start = 0; self._tick_reward_accum = 0.0
            return
        seg_end = len(self._train_obs)
        if seg_end > self._segment_start:
            batch_obs = np.asarray(self._train_obs[self._segment_start:seg_end], dtype=np.float32)
            acts_idx  = np.asarray(self._train_acts[self._segment_start:seg_end], dtype=np.int64)
            vals      = np.asarray(self._train_vals[self._segment_start:seg_end], dtype=np.float32)
            total_return = float(self._tick_reward_accum) + float(outcome_reward)
            returns = np.full_like(vals, total_return, dtype=np.float32)
            adv     = returns - vals
            self.step_update(batch_obs=batch_obs, acts_idx=acts_idx, returns=returns, adv=adv)
        self._train_obs.clear(); self._train_acts.clear(); self._train_vals.clear()
...
class HotswapACAdapter:
    ...
    def add_tick_reward(self, r: float) -> None:
        self.policy.add_tick_reward(float(r))

    def finalize_goal(self, goal_reward: float) -> None:
        self.policy.finalize_goal(float(goal_reward))
    ...
    def observe_only(self, state, team_agents: List[Any]) -> None:
        """Record an observation into the training buffer without changing profiles.

        Used during the cluster-replay cooldown window: the AC watches the game and
        accumulates (obs, act, val) tuples but is not allowed to reassign profiles yet.
        """
        self.policy.act(state, team_agents)  # side-effect: pushes to training buffer

...
#define MAX_LIST_SIZE_PER_PROC	20480000
...
void HyperCube_Class::check_list() {
    int tag = 0;
    int min_nbr = 100;		// Assumes list contains non-negative integers; 7-13-2026
    ...
    } else {					// Modified 7-13-2026
	my_min = min_nbr;			// Modified 7-13-2026
    }
    if (VERBOSE > 1) {
...
void HyperCube_Class::HyperCube_QuickSort() {
	...
	// ***** Add MPI call here *****
	MPI_Allreduce(&local_median, &pivot, 1, MPI_INT, MPI_SUM, sub_hypercube_comm); // pivot=sum([local_median]) then pivot=pivot/sub_hycube_sz to avg medians
	...
	if (nbr_k > my_id) {
	    ...
	    // MPI-3: Receive number of elements greater than or equal to pivot
	    ...
	    // MPI-6: Receive number of elements less than pivot

	    // ***** Add MPI call here *****
		MPI_Recv(&nbr_list_size, 1, MPI_INT, nbr_k, 0, MPI_COMM_WORLD, &status);
		
	    // MPI-7: Send number of elements greater than or equal to pivot
	    ...
	    // MPI-8: Receive neighbor's list of elements that are less than the pivot

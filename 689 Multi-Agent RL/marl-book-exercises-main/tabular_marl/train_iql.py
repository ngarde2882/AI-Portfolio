...
def iql_eval(env, config, q_tables, E, eval_episodes=500, output=True):
    """
    ...
    if mean_return[0] != 1:
        open("out.txt", "a").write(f"eval-{E}\n")
        open("out.txt", "a").write(f"Qa: {q_tables[0][str((0, 0))]}\t{q_tables[0][str((0, 1))]}\n")
        open("out.txt", "a").write(f"Qb: {q_tables[1][str((0, 0))]}\t{q_tables[1][str((0, 1))]}\n")
        open("out.txt", "a").write(f"R: {mean_return[0]}\t{mean_return[1]}\n\n")
    ...
    for eps_num in range(config["total_eps"]):
        ...
        if eps_num > 0 and eps_num % config["eval_freq"] == 0:
            mean_return, std_return = iql_eval(
                env, config, agents.q_tables, E, output=output
            )
...
if __name__ == "__main__":
    ...
    env = create_pd_game(ep_length=CONFIG["ep_length"])

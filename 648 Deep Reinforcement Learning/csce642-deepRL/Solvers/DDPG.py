...
class DDPG(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for _ in range(self.options.steps):
            ...
            # random batch of transitions B = {(s,a,r,s',d)} from D
            # compute targets (function)
            # update q by one gd step
            # update policy by one gd step
            self.replay() # only can trigger when enough samples exist in D
    ...
    def q_loss(self, current_q, target_q):
        ...
        #   YOUR IMPLEMENTATION HERE   #
        ################################
        return (target_q-current_q).pow(2)
